...
int run_child(int idx,client_data* users,char* share_mem){
    ...
    while(!stop_child)
    {
        int number=epoll_wait(child_epollfd,events,MAX_EVENT_NUMER,-1);
...
int main(int argc,char* argv[]){
	...
	setnonblocking(sig_pipfd[1]);
	setnonblocking(sig_pipfd[0]);
	...
	while ((!stop_server))
	{
		...
		for (int i = 0; i < number; i++)
		{
			...
			else if((sockfd==sig_pipfd[0])&&(events[i].events&EPOLLIN))
			{
				int sig;
				char signals[1024];
				ret=recv(sig_pipfd[0],signals,sizeof(signals),0);
				if(ret==-1){
					continue;
				}
				else if(ret==0){
